const _MTR_DATA = {
    "route": {
        "AEL": {
            "tc": "機場快綫",
            "en": "Airport Express",
            "color": "#1c7670"
        },
        "TCL": {
            "tc": "東涌綫",
            "en": "Tung Chung Line",
            "color": "#fe7f1d"
        },
        "TML": {
            "tc": "屯馬綫",
            "en": "Tuen Ma Line",
            "color": "#9a3b26"
        },
        "TKL": {
            "tc": "將軍澳綫",
            "en": "Tseung Kwan O Line",
            "color": "#6b208b"
        },
        "EAL": {
            "tc": "東鐵綫",
            "en": "East Rail Line",
            "color": "#53b7e8"
        },
        "SIL": {
            "tc": "南港島綫",
            "en": "South Island Line",
            "color": "#b5bd00"
        },
        "TWL": {
            "tc": "荃灣綫",
            "en": "Tsuen Wan Line",
            "color": "#ff0000"
        },
        "ISL": {
            "tc": "港島綫",
            "en": "Island Line",
            "color": "#0860a8"
        },
        "KTL": {
            "tc": "觀塘綫",
            "en": "Kwun Tong Line",
            "color": "#1a9431"
        },
        "DRL": {
            "tc": "迪士尼綫",
            "en": "Disneyland Resort Line",
            "color": "#f550a6"
        }
    },
    "station": {
    "HOK": {"tc": "香港", "en": "Hong Kong", "inter": ["TCL", "ISL", "TWL"]},
    "KOW": {"tc": "九龍", "en": "Kowloon"},
    "TSY": {"tc": "青衣", "en": "Tsing Yi"},
    "AIR": {"tc": "機場", "en": "Airport"},
    "AWE": {"tc": "博覽館", "en": "AsiaWorld Expo"},
    "OLY": {"tc": "奧運", "en": "Olympic"},
    "NAC": {"tc": "南昌", "en": "Nam Cheong", "inter": ["TCL", "TML"]},
    "LAK": {"tc": "荔景", "en": "Lai King", "inter": ["TCL", "TWL"]},
    "SUN": {"tc": "欣澳", "en": "Sunny Bay", "inter": ["TCL", "DRL"]},
    "TUC": {"tc": "東涌", "en": "Tung Chung"},
    "WKS": {"tc": "烏溪沙", "en": "Wu Kai Sha"},
    "MOS": {"tc": "馬鞍山", "en": "Ma On Shan"},
    "HEO": {"tc": "恆安", "en": "Heng On"},
    "TSH": {"tc": "大水坑", "en": "Tai Shui Hang"},
    "SHM": {"tc": "石門", "en": "Shek Mun"},
    "CIO": {"tc": "第一城", "en": "City One"},
    "STW": {"tc": "沙田圍", "en": "Sha Tin Wai"},
    "CKT": {"tc": "車公廟", "en": "Che Kung Temple"},
    "TAW": {"tc": "大圍", "en": "Tai Wai", "inter": ["TML", "EAL"]},
    "HIK": {"tc": "顯徑", "en": "Hin Keng"},
    "DIH": {"tc": "鑽石山", "en": "Diamond Hill", "inter": ["TML", "KTL"]},
    "KAT": {"tc": "啟德", "en": "Kai Tak"},
    "SUW": {"tc": "宋皇臺", "en": "Sung Wong Toi"},
    "TKW": {"tc": "土瓜灣", "en": "To Kwa Wan"},
    "HOM": {"tc": "何文田", "en": "Ho Man Tin", "inter": ["TML", "KTL"]},
    "HUH": {"tc": "紅磡", "en": "Hung Hom", "inter": ["TML", "EAL"]},
    "ETS": {"tc": "尖東", "en": "East Tsim Sha Tsui", "inter": ["TWL", "TML"]},
    "AUS": {"tc": "柯士甸", "en": "Austin"},
    "MEF": {"tc": "美孚", "en": "Mei Foo", "inter": ["TML", "TWL"]},
    "TWW": {"tc": "荃灣西", "en": "Tsuen Wan West"},
    "KSR": {"tc": "錦上路", "en": "Kam Sheung Road"},
    "YUL": {"tc": "元朗", "en": "Yuen Long"},
    "LOP": {"tc": "朗屏", "en": "Long Ping"},
    "TIS": {"tc": "天水圍", "en": "Tin Shui Wai"},
    "SIH": {"tc": "兆康", "en": "Siu Hong"},
    "TUM": {"tc": "屯門", "en": "Tuen Mun"},
    "NOP": {"tc": "北角", "en": "North Point", "inter": ["TKL", "ISL"]},
    "QUB": {"tc": "鰂魚涌", "en": "Quarry Bay", "inter": ["TKL", "ISL"]},
    "YAT": {"tc": "油塘", "en": "Yau Tong", "inter": ["TKL", "KTL"]},
    "TIK": {"tc": "調景嶺", "en": "Tiu Keng Leng", "inter": ["TKL", "KTL"]},
    "TKO": {"tc": "將軍澳", "en": "Tseung Kwan O"},
    "LHP": {"tc": "康城", "en": "LOHAS Park"},
    "HAH": {"tc": "坑口", "en": "Hang Hau"},
    "POA": {"tc": "寶琳", "en": "Po Lam"},
    "ADM": {"tc": "金鐘", "en": "Admiralty", "inter": ["EAL", "SIL", "TWL", "ISL"]},
    "EXC": {"tc": "會展", "en": "Exhibition Centre"},
    "MKK": {"tc": "旺角東", "en": "Mong Kok East"},
    "KOT": {"tc": "九龍塘", "en": "Kowloon Tong", "inter": ["EAL", "KTL"]},
    "SHT": {"tc": "沙田", "en": "Sha Tin"},
    "FOT": {"tc": "火炭", "en": "Fo Tan"},
    "RAC": {"tc": "馬場", "en": "Racecourse"},
    "UNI": {"tc": "大學", "en": "University"},
    "TAP": {"tc": "大埔墟", "en": "Tai Po Market"},
    "TWO": {"tc": "太和", "en": "Tai Wo"},
    "FAN": {"tc": "粉嶺", "en": "Fanling"},
    "SHS": {"tc": "上水", "en": "Sheung Shui"},
    "LOW": {"tc": "羅湖", "en": "Lo Wu"},
    "LMC": {"tc": "落馬洲", "en": "Lok Ma Chau"},
    "OCP": {"tc": "海洋公園", "en": "Ocean Park"},
    "WCH": {"tc": "黃竹坑", "en": "Wong Chuk Hang"},
    "LET": {"tc": "利東", "en": "Lei Tung"},
    "SOH": {"tc": "海怡半島", "en": "South Horizons"},
    "CEN": {"tc": "中環", "en": "Central", "inter": ["TWL", "ISL"]},
    "TST": {"tc": "尖沙咀", "en": "Tsim Sha Tsui", "inter": ["TWL", "TML"]},
    "JOR": {"tc": "佐敦", "en": "Jordan"},
    "YMT": {"tc": "油麻地", "en": "Yau Ma Tei", "inter":["TWL", "KTL"]},
    "MOK": {"tc": "旺角", "en": "Mong Kok", "inter":["TWL", "KTL"]},
    "PRE": {"tc": "太子", "en": "Prince Edward", "inter":["TWL", "KTL"]},
    "SSP": {"tc": "深水埗", "en": "Sham Shui Po"},
    "CSW": {"tc": "長沙灣", "en": "Cheung Sha Wan"},
    "LCK": {"tc": "荔枝角", "en": "Lai Chi Kok"},
    "KWF": {"tc": "葵芳", "en": "Kwai Fong"},
    "KWH": {"tc": "葵興", "en": "Kwai Hing"},
    "TWH": {"tc": "大窩口", "en": "Tai Wo Hau"},
    "TSW": {"tc": "荃灣", "en": "Tsuen Wan"},
    "KET": {"tc": "堅尼地城", "en": "Kennedy Town"},
    "HKU": {"tc": "香港大學", "en": "HKU"},
    "SYP": {"tc": "西營盤", "en": "Sai Ying Pun"},
    "SHW": {"tc": "上環", "en": "Sheung Wan"},
    "WAC": {"tc": "灣仔", "en": "Wan Chai"},
    "CAB": {"tc": "銅鑼灣", "en": "Causeway Bay"},
    "TIH": {"tc": "天后", "en": "Tin Hau"},
    "FOH": {"tc": "炮台山", "en": "Fortress Hill"},
    "TAK": {"tc": "太古", "en": "Tai Koo"},
    "SWH": {"tc": "西灣河", "en": "Sai Wan Ho"},
    "SKW": {"tc": "筲箕灣", "en": "Shau Kei Wan"},
    "HFC": {"tc": "杏花邨", "en": "Heng Fa Chuen"},
    "CHW": {"tc": "柴灣", "en": "Chai Wan"},
    "WHA": {"tc": "黃埔", "en": "Whampoa"},
    "SKM": {"tc": "石硤尾", "en": "Shek Kip Mei"},
    "LOF": {"tc": "樂富", "en": "Lok Fu"},
    "WTS": {"tc": "黃大仙", "en": "Wong Tai Sin"},
    "CHH": {"tc": "彩虹", "en": "Choi Hung"},
    "KOB": {"tc": "九龍灣", "en": "Kowloon Bay"},
    "NTK": {"tc": "牛頭角", "en": "Ngau Tau Kok"},
    "KWT": {"tc": "觀塘", "en": "Kwun Tong"},
    "LAT": {"tc": "藍田", "en": "Lam Tin"},
    "DIS": {"tc": "迪士尼", "en": "Disneyland Resort"}
},

    "route_stn": {
      "ISL": ["CHW","HFC","SKW","SWH","TAK","QUB","NOP","FOH","TIH","CAB","WAN","ADM","CEN","SHW","SYP","HKU","KET"],
      "TWL": ["TSW", "TWH", "KWH", "KWF", "LAK", "MEF", "LCK", "CSW", "SSP", "PRE", "MOK", "YMT", "JOR", "TST", "ADM", "CEN"],
      "KTL": ["TIK","YAT","LAT","KWT","NTK","KOB","CHH","DIH","WTS","LOF","KOT","SKM","PRE","MOK","YMT","HOM","WHA"],
      "TKL": ["POA","HAH","LHP","TKO","TIK","YAT","QUB","NOP"],
      "TCL": ["TUC","SUN","LAK","NAC","OLY","KOW","HOK"],
      "DRL": ["SUN", "DIS"],
      "AEL": ["AWE", "AIR", "TSY", "KOW", "HOK"],
      "EAL": ["LOW", "LMC", "SHS", "FAN", "TWO", "TAP", "UNI", "RAC", "FOT", "SHT", "TAW", "KOT", "MKK", "HUH", "EXC", "ADM"],
      "TML": ["TUM", "SIH", "TIS", "YUL", "LOP", "KSR", "TWW", "MEF", "NAC", "AUS", "ETS", "HUH", "HOM", "TKW", "SUW", "KAT", "DIH", "HIK", "TAW", "CKT", "STW", "CIO", "SHM", "TSH", "HEO", "MOS", "WKS"],
      "SIL": ["SOH", "LET", "WCH", "OCP", "ADM"]
    },

    "exp_from":{
        "CEN":{
            "ISL+KET":[[["4-2"],"TWL","TSW"]],
            "TWL+TSW":[[["1-1"],"ISL","KET"]]
        },
        "NOP":{
            "ISL+KET":[[["1-3","6-4","8-4"],"TKL","POA"]],
            "TKL+POA":[[["1-4","5-4","7-1"],"ISL","KET"]],
        },
        "QUB":{
            "ISL+CHW":[[["6-4"],"TKL","POA"]],
            "TKL+POA":[[["4-5"],"ISL","CHW"]],
        },
        
        "ADM":{
            "ISL+KET":[[["5-1"],"SIL","SOH"], [["3-1","5-1"],"EAL","LOW"]],
            "SIL+SOH":[[["1-4"],"ISL","KET"], [["1-4"],"ISL","CHW"],[["1-4"],"TWL","TSW"], [["1-4"],"EAL","LOW"]],
            "TWL+TSW":[["N/A","ISL","CHW"],[["4-4"],"SIL","SOH"]],
            "ISL+CHW":[["N/A","TWL","TSW"],[["4-5"],"SIL","SOH"],[["4-5","6-5"],"EAL","LOW"]]
        },
        
        "YAT":{
            "TKL+NOP":[["N/A","KTL","WHA"]],
            "KTL+WHA":[["N/A","TKL","NOP"]],
        },
        "TIK":{
            "TKL+POA":[["N/A","KTL","WHA"]],
            "KTL+WHA":[["N/A","TKL","POA"]],
        },
        "HUH":{

        },
        "YMT":{
            "TWL+CEN":[[["3-5","7-5"],"KTL","WHA"]],
            "KTL+WHA":[[["5-5"],"TWL","CEN"]],
        },
        "MOK":{
            "TWL+CEN":[["N/A","KTL","TIK"]],
            "KTL+TIK":[["N/A","TWL","CEN"]],
        },
        "PRE":{
            "TWL+TSW":[["N/A","KTL","TIK"]],
            "KTL+WHA":[["N/A","TWL","CEN"]],
        },

        "KOT":{
            "EAL+ADM":[[["3-4","9-2"],"KTL","WHA"],[["3-4","9-2"],"KTL","TIK"]],
            "EAL+LOW":[[["1-1","3-2"],"KTL","WHA"],[["1-1","3-2"],"KTL","TIK"]],
            "KTL+TIK":[[["3-1"],"EAL","ADM"],[["3-1"],"EAL","LOW"]],
            "KTL+WHA":[[["6-5"],"EAL","ADM"],[["6-5"],"EAL","LOW"]],
        },
    }
  }